...
<div class="features_items" ng-controller="productController" ng-init="findByCategory(${categoryId},$scope.page, $scope.size); cartUrl='${ctx.contextPath}/${ctx.baseSite}/Cart.html'"">
	<!--features_items-->
	...
	<div class="col-sm-12 clearfix">
		...
		<ul class="pagination">
		    <li><a href="" ng-click="findByCategory(${categoryId},product.number - 1,product.size)" ng-show="product.number > 0">&laquo;</a></li>
		    <li ng-repeat="i in range(product.totalPages)" ng-class="{active: product.number == $index}"><a href="" ng-click="findByCategory(${categoryId},$index,product.size)">{{$index+1}}</a></li>
		    <li><a href="" ng-click="findByCategory(${categoryId},product.number + 1,product.size)" ng-show="product.number < product.totalPages - 1">&raquo;</a></li>
		</ul>
		</div>
