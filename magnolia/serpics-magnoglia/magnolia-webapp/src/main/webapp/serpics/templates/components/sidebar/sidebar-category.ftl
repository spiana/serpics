...
<div class="left-sidebar" ng-controller="categoryController" ng-init="getTop()">
		...
		<ul>
			<li >
        		...
        		<h4 class="panel-title">
        			<a href="${ctx.contextPath}/${ctx.baseSite}/Category?categoryId={{category.id}}&categoryName={{category.name || category.code}}">
        			...
        			<div class="panel-body">
						<ul >
                			<li ng-if="category.childCategoryNumber !=0" ng-repeat="subCat in categoryData[$index].subCategory"  ng-show="category.active">
                    			<a href="${ctx.contextPath}/${ctx.baseSite}/Category?categoryId={{subCat.id}}&categoryName={{subCat.name || subCat.code}}">{{subCat.name || subCat.code}}</a>
                			</li>
            			</ul>
            		</div>
            	</div>
