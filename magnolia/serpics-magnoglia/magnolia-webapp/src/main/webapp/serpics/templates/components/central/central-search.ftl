...
<div class="features_items" ng-controller="productController" ng-init="findBySearch('${textSearch}',$scope.page, $scope.size); cartUrl='${ctx.contextPath}/${ctx.baseSite}/Cart'"">
	...
	<div class="col-sm-12 clearfix">
		...
		<ul class="pagination">
		    <li><a href="" ng-click="findBySearch(${textSearch},0,product.size)" ng-show="product.number > 0">&laquo;</a></li>
		    <li><a href="" ng-click="findBySearch(${textSearch},product.number - 1,product.size)" ng-show="product.number > 0">&lsaquo;</a></li>
		    <li ng-repeat="i in range(product.totalPages)" ng-class="{active: product.number == $index}" ng-show="((((product.number - $index > -3) || $index < 5) && product.number < $index) || (((product.number - $index < 3) || $index > (product.totalPages - 6)) && product.number > $index) || (product.number == $index))"><a href="" ng-click="findBySearch(${textSearch},$index,product.size)">{{$index+1}}</a></li>
		    <li><a href="" ng-click="findBySearch(${textSearch},product.number + 1,product.size)" ng-show="product.number < product.totalPages - 1">&rsaquo;</a></li>
		    <li><a href="" ng-click="findBySearch(${textSearch},product.totalPages - 1,product.size)" ng-show="product.number < product.totalPages - 1">&raquo;</a></li>
		</ul>
		</div>
