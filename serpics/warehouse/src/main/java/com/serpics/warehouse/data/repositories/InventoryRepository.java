...
public interface InventoryRepository extends Repository<Inventory, Long>{
	...
	public Double getAvailable(@Param("product")BaseProduct product , @Param("store") Store store);
	
	
	@Query("select sum(i.available - i.reserved) from Inventory i where i.warehouse=:warehouse and i.product=:product")
	public Double getAvailableByWarehouse(@Param("product")BaseProduct product , @Param("warehouse") Warehouse warehouse);
}
