...
public class HookScannerPostProcessor implements BeanDefinitionRegistryPostProcessor {
				...
				try {
					...
					if (hookImpls == null) {
						...
						if (hookImpls.containsKey(store))
							logger.warn("Ovveride implementation for HOOK {} and STORE {} : class " + clazz, hook,
									store);
						// throw new
						// FatalBeanException("Duplicate implementation found for HOOK "
						// + hook + ". Found classes: " + clazz + ", " +
						// hookImpls.get(store).getName());
						hookImpls.put(store, Class.forName(clazz));
					}
	...
	public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) throws BeansException {
		...
		for (String hook : hookInterfaceMap.keySet()) {
			...
			// hookImplementationMap.get(hook));
