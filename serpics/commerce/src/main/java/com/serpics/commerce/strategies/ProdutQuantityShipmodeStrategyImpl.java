...
@StoreStrategy("productQtyShipmodeStrategy")
public class ProdutQuantityShipmodeStrategyImpl implements ShipmodeStrategy{
	    	...
	    	if (order.getShipmode() != null){
		    		...
		    		if (lookup.getZipcode() != null){
		    			...
		    			if (shipmodelookup != null){
		    				if (shipmodelookup.getStore() == null){
				    			if (lookup.getStore().getId().equals(order.getStore().getId())){
					    			shipmodelookup = lookup;
				    			}
		    				}
		    			} else {
		    	...
		    	if (shipmodelookup != null){
			    	for (Shipping shipping : shipmodelookup.getShippings()){
			    		if (shipping.getRangestart() <= totalQuantity && (selectedShipping.getRangestart() == null || (shipping.getRangestart() > selectedShipping.getRangestart()))){
